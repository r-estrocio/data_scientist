...
def shortest_handle
  $handles.min_by { |h| h.length }
end

# Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def handles_5chars_count
  $handles.count { |h| h.length == 6 }
end

# Combien commencent par une majuscule (première lettre juste après le @) ?
def uppercase_handles_count
  $handles.count { |h| h[1] == h[1].upcase }
end

# Trie la liste de handle par ordre alphabétique.
def handles_alphabetical_order
  $handles.sort_by { |h| h.downcase }
end

# Trie la liste de handle par taille des handle (les plus petits en premiers,
# les plus grands après)
def handles_length_order
  $handles.sort_by { |h| h.length }
end

# Quelle est la position dans l'array de la personne @epenser ?
def handle_position_for_epenser
  $handles.index("@epenser") + 1
end

# Sors-moi une répartition des handle par taille de ces derniers
# (nombre de handle avec 1 caractère,
# nombre de handle avec 2 caractères,
# nombre de handle avec 3 caractères, etc)
def handles_group_by_length
  $handles.group_by { |h| h.length - 1 }
end

puts "Le array a #{handles_count} handles ."
puts
...
puts
puts "Liste de handles par ordre de taille : #{handles_length_order.join(" ")}"
puts
puts "Liste de handles par ordre alphabétique : #{handles_alphabetical_order.join(" ")}"
puts
puts "La position de @epenser est #{handle_position_for_epenser}."
puts
handles_lengths = handles_group_by_length.keys
handles_lengths_ordered = handles_lengths.sort
handles_lengths_ordered.each do |length|
  handles_with_length = handles_group_by_length[length]
  puts "Il y a #{handles_with_length.count} handles avec #{length} caractères."
end
